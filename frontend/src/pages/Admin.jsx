...
export default function Admin() {
  ...
  const openModal = (mujer = null) => {
    if (mujer) {
      ...
      setFormData({
        ...
        es_seudonimo: mujer.es_seudonimo === 1,
        seudonimo: mujer.seudonimo || ''
      });
    } else {
      setEditando(null);
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    for (const key in formData) {
      ...
      } else if (key === 'esLider' || key === 'es_seudonimo') {
              ...
              <div className={styles.formGrid}>
                ...
                <div className={styles.grupo}>
                  <label className={styles.labelCheckbox}>
                    <input
                      type="checkbox"
                      checked={formData.es_seudonimo}
                      onChange={(e) => setFormData({...formData, es_seudonimo: e.target.checked})}
                    />
                    Pseudonimo
                  </label>
                </div>
                {formData.es_seudonimo && (
                  <div className={styles.grupo}>
                    <label className={styles.label}>Nombre real (opcional)</label>
                    <input
                      type="text"
                      value={formData.seudonimo}
                      onChange={(e) => setFormData({...formData, seudonimo: e.target.value})}
                      className={styles.input}
                      placeholder="Ej: Carolina Herrera Pantoja"
                    />
                  </div>
                )}
              </div>
